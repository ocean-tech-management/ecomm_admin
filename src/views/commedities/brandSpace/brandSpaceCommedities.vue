  ...
  <div class="subpage-content">
    <div class="default-container">
      ...
      <div class="table-ctrls">
        <div class="table-title">品牌商品列表 <span class="success-color">{{activeData.title || ''}}</span></div>
      ...
      <el-table style="width: 100%" :data="dataList">
        ...
        <el-table-column label="操作" width="180">
         <template slot-scope="scope">
           ...
           <template v-if="$auths.includes('brandSpace/goodsSort')">
            <el-button type="text" size="default" @click="showSort(scope.row)">排序</el-button>
           </template>
            <template v-if="$auths.includes('brandSpace/goodsUpdate')">
              <el-button type="text" size="small" @click="showEdit(scope.row)">编辑</el-button>
            </template>
            <template v-if="$auths.includes('brandSpace/goodsDelete')">
    ...
    <el-dialog title="新增品牌商品" width="1200px" :close-on-click-modal="false" :visible.sync="addVisible">
    ...
    <el-dialog title="编辑品牌商品" width="1200px" :close-on-click-modal="false" :visible.sync="editVisible">
    ...
    <!-- 品牌馆商品排序 -->
    <el-dialog title="品牌馆商品排序" width="750px" :close-on-click-modal="false" :visible.sync="sortVisible">
...
import AddCommedities from '@/partialviews/commedity/brandSpace/commedity/AddCommedities';
import EditCommedities from '@/partialviews/commedity/brandSpace/commedity/EditCommedities';
import moment from 'moment';

export default {
  name: 'brandSpaceCommedities',
  components: {
    AddCommedities,
    EditCommedities
  },
  ...
  mounted() {
    ...
    if(!this.aId) {
      this.$message.error('品牌商品不存在或已失效，正在返回');
      setTimeout(() => {
  ...
  methods: {
    ...
    getList() {
      ...
      this.$get('/admin/brandSpace/goodsInfo', queryVal, res => {
    ...
    showAdd() {
      if(!this.$auths.includes('spu/list')) {
        return this.$message.warning('您缺少查看商品列表的权限，无法正常添加品牌商品，请联系管理员为您添加'); 
      }
      let formData = {
    ...
    addItems() {
      ...
      for(let i = 0; i < params.goods.length; i++) {
        if(
          ...
          return this.$message.error(`第${i + 1}个品牌商品的价格要为大于零的数哦，请检查下哈`)
        }
        if(
          ...
          return this.$message.error(`第${i + 1}个品牌商品的价格需小于销售价且大于最高成本价哦，请检查下哈`)
        }
        if(
        ...
        if(!saleNumberGoods.includes(params.goods[i].goods_sn)) {
          ...
          if(params.goods[i].sale_number !== params.sale_number[params.goods[i].goods_sn]) return this.$message.error(`第${i + 1}个品牌商品的在选中商品列表中有同款，请将两者的起始销量设为一致`)
        }
        ...
        if(
          ...
          return this.$message.error(`第${i + 1}品牌商品的成长值要为大于零的数哦，请检查下哈`)
        }
        ...
        if(!vipLevelGoods.includes(params.goods[i].goods_sn)) {
          ...
          if(params.goods[i].vip_level !== params.vip_level[params.goods[i].goods_sn]) this.$message.error(`第${i + 1}个品牌商品的在选中商品列表中有同款，请将两者的最高可用等级设为一致`)
        }
      }
      
      this.$post('/admin/brandSpace/goodsUpdate', params, res => {
    ...
    showEdit(row) {
      if(!this.$auths.includes('brandSpace/goodsSkuInfo')) {
        return this.$message.warning('您缺少查看详情的权限，无法正常修改品牌商品，请联系管理员为您添加'); 
      }
      let params = {
        aId: this.aId,
      ...
      this.$get('/admin/brandSpace/goodsSkuInfo', params, res => {
        let data = res.data; 
        let formData = {
    ...
    editItem() {
      ...
      if(
        ...
        return this.$message.error(`品牌商品的起始销量要为0,1,2,3,4之类的整数哦，请检查下哈`)
      }
      for(let i = 0; i < params.goods.length; i++) {
        if(
          !(/^(\d|[1-9]\d+)(\.\d+)?$/.test(params.goods[i].activity_price)) ||
          ...
          return this.$message.error(`第${i + 1}品牌商品的价格要为大于零的数哦，请检查下哈`)
        }
    ...
    deleteItem(row) {
      this.$confirm(`您将删除${row.title}, 是否继续?`, '提示', {
        ...
        this.$post('/admin/brandSpace/goodsDelete', {
          id: row.id
        }, res => {
    ...
    confirmSortInfo() {
      ...
      this.$post('/admin/brandSpace/goodsSort', params, res => {
