    ...
    <div class="default-container">
      ...
      <div class="table-ctrls">
        ...
        <div class="table-ctrl">
          <template v-if="$auths.includes('pt/create')">
      ...
      <el-table style="width: 100%" :data="dataList">
        ...
        <el-table-column label="成团时限" width="80" align="center">
          <template slot-scope="scope">
            {{scope.row.expire_time_val}}{{expireTypeText[scope.row.expire_time_type]}}
          </template>
        </el-table-column>
        <el-table-column label="每人可参加" width="100" align="center">
          <template slot-scope="scope">
            {{scope.row.join_number}}次
          </template>
        </el-table-column>
        ...
        <el-table-column label="操作" width="240">
         <template slot-scope="scope">
           <template v-if="$auths.includes('pt/status')">
          ...
          <template v-if="$auths.includes('pt/update')">
            <el-button type="text" size="small" @click="showEdit(scope.row)">编辑</el-button>
          </template>
          <template v-if="$auths.includes('pt/goodsInfo')">
            <el-button type="text" size="small" @click="toRelations(scope.row)">查看关联商品</el-button>
          </template>
          <template v-if="$auths.includes('pt/delete')">
...
import Add from '@/partialviews/commedity/collage/Add';
import Edit from '@/partialviews/commedity/collage/Edit';
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      this.$get('/admin/pt/list', queryVal, res => {
    ...
    addItem() {
      ...
      if(
        ...
        !(/^[1-9]\d*$/.test(params.join_number)) ||
        !(/^[1-9]\d*$/.test(params.sort))
        ) {
        return this.$message.error(`拼团有效时间、成团人数要求、每人可参加次数、排序都要为1，2，3，4这种正整数哦`)
      }

      this.$post('/admin/pt/create', params, res => {
    ...
    toRelations(row) {
      this.$router.push('/collageCommedities?aCode=' + row.activity_code);
    },
  }
};
</script>
