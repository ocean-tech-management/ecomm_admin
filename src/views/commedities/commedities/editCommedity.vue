<template>
  <el-form :model="formData">
    ...
    <div class="subpage-content" id="goods_base">
      <div class="default-container">
        <el-form-item label="商品名称" label-width="100px">
          <el-input auto-complete="off" placeholder="填一下商品的名称呗" v-model="formData.title"></el-input>
        </el-form-item>
        <el-form-item label="商品编码" label-width="100px">
          <el-input auto-complete="off" placeholder="填一下商品的商品码呗" maxlength="36" v-model="formData.goods_code"></el-input>
        </el-form-item>
        <el-form-item label="商品简介" label-width="100px">
          <el-input auto-complete="off" placeholder="填一下商品的简介呗" v-model="formData.sub_title"></el-input>
        </el-form-item>

        <!-- <el-form-item label="商品编号" label-width="100px">
          <div class="form-content-static">{{formData.goods_sn}}</div>
        </el-form-item> -->
        ...
        <el-form-item v-if="$auths.includes('tag/list')" label="关联标签" label-width="100px" prop="tag_id">
        ...
        <el-form-item v-if="$auths.includes('supplier/list')" label="供应商" label-width="100px" prop="supplier_code">
        ...
        <el-form-item v-if="$auths.includes('goods/postageTemplates')" label="运费模板" label-width="100px"
          prop="postage_code">
          <el-select placeholder="不限" name="postage_code" filterable remote :loading="fareTemplateLoading"
            v-model="formData.postage_code" :remote-method="updateFareTemplate">
        ...
        <el-form-item label="发货仓" label-width="100px" prop="warehouse_icon">
          <el-select placeholder="集品严选自有仓" name="warehouse_icon" v-model="formData.warehouse_icon">
        ...
        <el-form-item label="附加条件" label-width="100px">
          <el-select placeholder="是否有特殊要求" name="attach_type" v-model="formData.attach_type" @change="$forceUpdate()">
        ...
        <el-form-item label="内页广告图" label-width="100px">
          <div class="commedity-banners">
            <div v-for="(item, index) in formData.images" class="commedity-banner" :key="item.code">
              <template v-if="($auths.includes('goods/deleteImages') && item.id) || !item.id">
    ...
    <div class="subpage-content">
      <div class="default-container">
        <el-form-item label="商品视频" label-width="100px">
          ...
          <div style="color: #999; font-size:14px; line-height: 1.4; margin-top: 10px">请上传尺寸不超过20M的视频</div>
        </el-form-item>
      </div>
    </div>
      ...
      <div class="default-container">
        ...
        <div v-show="selectCategory.code">
          ...
          <el-form-item label label-width="300px">
            <el-button type="primary" @click="askConfirmStandard">确定规格</el-button>
          </el-form-item>
        </div>
    ...
    <div v-show="skuList && skuList.length > 0" class="subpage-content" id="goods_sku">
      <div class="default-container">
        <div class="table-ctrls">
          ...
          <div class="table-ctrl">
            <template v-if="$auths.includes('goods/priceMaintain')">
              <el-switch v-model="priceLock" style="margin: 10px 20px 0 0" active-text="锁定sku" active-color="#ff4949"
                inactive-color="#ccc" :disabled="defaultPriceVisible" @change="tryToggleLock">
              </el-switch>
            </template>
        ...
        <el-form-item label-width="0">
          <el-table style="width: 100%" :data="skuList" default-expand-all>
                ...
                <el-form v-if="scope.row.purchase_prices" label-width="100px" inline>
                  <div v-for="(vdcItem, vdcIndex) in scope.row.purchase_prices" class="sku-vdc-item" :key="vdcIndex">
                    <el-form-item :label="vdcItem.name + '进货价'">
                      <div v-show="scope.row.sku_sn && priceLock" class="sku-vdc-info-static">{{ vdcItem.purchase_price }}
                      </div>
                      <el-input v-show="!scope.row.sku_sn || !priceLock" style="width: 120px" type="number"
                        auto-complete="off" :placeholder="vdcItem.name + '进货价'"
                    ...
                    <el-form-item v-show="vdcItem.close_divide === 2" :label="vdcItem.name + '一级分润'">
                      <div v-show="scope.row.sku_sn && priceLock" class="sku-vdc-info-static">{{ vdcItem.vdc_one }}</div>
                      <el-input v-show="!scope.row.sku_sn || !priceLock" style="width: 120px" auto-complete="off"
                        :placeholder="vdcItem.name + '一级分润'"
                    ...
                    <el-form-item v-show="vdcItem.close_divide === 2" :label="vdcItem.name + '二级分润'">
                      <div v-show="scope.row.sku_sn && priceLock" class="sku-vdc-info-static">{{ vdcItem.vdc_two }}</div>
                      <el-input v-show="!scope.row.sku_sn || !priceLock" style="width: 120px" auto-complete="off"
                        :placeholder="vdcItem.name + '二级分润'"
            ...
            <!-- <el-table-column label="库存" width="150">
              <template slot-scope="scope">
                <el-input style="width: 120px" auto-complete="off" placeholder="填一下" v-model="skuList[scope.$index].stock" type="number"></el-input>
              </template>
            </el-table-column> -->
            <el-table-column label="库存" width="130" prop="stock">
              <template slot-scope="scope">
                <div v-show="scope.row.sku_sn">{{ scope.row.stock }}</div>
                <el-input v-show="!scope.row.sku_sn" style="width: 110px" auto-complete="off" placeholder="填一下"
                  v-model="skuList[scope.$index].stock" type="number"></el-input>
              </template>
            </el-table-column>
            <el-table-column label="销售价格" width="130">
              <template slot-scope="scope">
                <div v-show="scope.row.sku_sn && priceLock" class="">{{ scope.row.sale_price }}</div>
                <el-input v-show="!scope.row.sku_sn || !priceLock" style="width: 110px" auto-complete="off"
                  placeholder="填一下" v-model="skuList[scope.$index].sale_price" type="number"></el-input>
              </template>
            </el-table-column>
            <!-- <el-table-column label="会员价" width="150">
              <template slot-scope="scope">
                <el-input style="width: 120px" auto-complete="off" placeholder="填一下" v-model="skuList[scope.$index].member_price" type="number"></el-input>
              </template>
            </el-table-column> -->

            <el-table-column label="市场价" width="130">
              <template slot-scope="scope">
                <div v-show="scope.row.sku_sn && priceLock" class="">{{ scope.row.market_price }}</div>
                <el-input v-show="!scope.row.sku_sn || !priceLock" style="width: 110px" auto-complete="off"
                  placeholder="填一下" v-model="skuList[scope.$index].market_price" type="number"></el-input>
              </template>
            </el-table-column>
            <el-table-column label="成本价" width="130">
              <template slot-scope="scope">
                <div v-show="scope.row.sku_sn && priceLock" class="">{{ scope.row.cost_price }}</div>
                <el-input v-show="!scope.row.sku_sn || !priceLock" style="width: 110px" auto-complete="off"
                  placeholder="填一下" v-model="skuList[scope.$index].cost_price" type="number"></el-input>
              </template>
            </el-table-column>
            <el-table-column label="操作" width="70">
              <template slot-scope="scope">
                <!-- 新数据或者有删除权限的时候显示 -->
                <template v-if="($auths.includes('goods/deleteSku') && scope.row.sku_sn) || !scope.row.sku_sn">
        ...
        <el-form-item label="详细描述" prop="desc" label-width="100px">
          ...
          <div>
            <el-upload id="img-uploader" class="thumbnail-uploader" accept="image/png, image/jpeg, image/gif"
              action="/admin/file/upload" ref="upload" :data="thumbData" :headers="token" :show-file-list="false"
              :before-upload="uploadWatcher" :on-error="uploadError" :on-exceed="overLimit" :limit="5" auto-upload
              multiple></el-upload>
          </div>
        </el-form-item>
        <el-form-item label label-width="300px">
          <el-button type="primary" @click="beforeEdit">保存</el-button>
        </el-form-item>
      </div>
    </div>
    ...
    <el-dialog :append-to-body="true" title="新增规格值" width="600px" :close-on-click-modal="false"
    ...
    <el-dialog title="批量填充sku信息" width="600px" :close-on-click-modal="false" :visible.sync="defaultPriceVisible">
...
export default {
  ...
  mounted() {
    if(!this.$route.query.goods_sn) {
      this.$message.error('所选择商品有问题，将返回列表');
      return this.$router.replace('commedities');
    }
    if(!this.$auths.includes('goods/info')) {
      return this.$message.warning('您缺少查看商品详情的权限，请联系管理员为您添加');
    }
    if(!this.$auths.includes('goods/postageTemplates')) {
      this.$message.warning('您缺少查看商品邮费模板的权限，无法正常编辑商品，请联系管理员为您添加');
      return this.$router.replace('commedities');
    }
    this.getTags();
    this.getVDC(() => {
  ...
  methods: {
    getData() {
      ...
      this.$get('/admin/goods/info', params, res => {
        let data = res.data;
        this.formData = {
          ...
          show_status: data.show_status || 1,
          tag_id: data.tag_id || '',
          ...
          warehouse_icon: data.warehouse_icon && data.warehouse_icon !== '' ? parseInt(data.warehouse_icon) : '',
          desc: data.desc,
          up_type: data.up_type
        }

        for(let i = 0; i < data.goodsImages.length; i++) {
          let item = data.goodsImages[i];
          item.code = new Date().getTime() + i;
          this.formData.images.push(item);
        }
        if(data.goods_videos) {
          for(let i = 0; i < data.goods_videos.length; i++) {
            let item = data.goods_videos[i];
            item.code = new Date().getTime() + i;
            this.formData.videos.push(item);
          }
        }

        this.goodsParams = data.params ? JSON.parse(data.params || '[]') : [];

        if(data.up_type) {
          if(data.up_time) this.upTimes.up_time = data.up_time;
          if(data.down_time) this.upTimes.down_time = data.down_time;
        }
        ...
        this.formData.attach_type = data.sku[0].attach_type;
        this.formData.postage_code = data.sku[0].postage_code;
        if(data.sku[0].price_maintain != 1) this.priceLock = false;
        ...
        for(let i = 0; i < data.sku.length; i++) {
          // 检查sku列表，检查哪些sku属性被选中了
          let standards = JSON.parse(data.sku[i].specs);
          let standardValues = Object.values(standards);
          ...
          for(let j = 0; j < this.vdc.length; j++) {
            purchase_prices.push({
              ...
              vdc_one: data.sku[i].vdc[j].vdc_one,
              vdc_two: data.sku[i].vdc[j].vdc_two,
          ...
          skuList.push({
            ...
            sku_sn: data.sku[i].sku_sn,
            title: data.sku[i].title,
            image: data.sku[i].image,
            supplier_code: data.supplier_code,
            standards: standardValues, //skuArr的长度为1时，是一维数组
            stock: data.sku[i].stock,
            virtual_stock: data.sku[i].virtual_stock,
            sale_price: data.sku[i].sale_price,
            member_price: data.sku[i].member_price,
            market_price: data.sku[i].market_price,
            cost_price: data.sku[i].cost_price,
            purchase_prices: JSON.parse(JSON.stringify(purchase_prices))  //0622，加入不同身份不同进货价板块
          })
        }

        let standards = JSON.parse(data.sku[0].specs);
        let standardKeys = Object.keys(standards);
        this.skuStandards.push(...standardKeys);
    ...
    tryToggleLock(val) {
      ...
      if(!val) confirmTexts = '您将进行解锁spu操作，解锁spu后将允许后台管理人员对sku的价格进行调整，在重新锁定sku前将会限制用户购买';
    ...
    resetStandards() {
      ...
      this.getStandards(() => {
        ...
        this.standardNameSelected = [
          this.standards[0].attribute_name || ''
        ];
      });
    ...
    askConfirmStandard() {
      // 20211124，直接重置sku了，局部新增的话直接使用新增sku即可
      this.$confirm(('你的操作会重置整个产品列表，此操作将会影响在售商品，且操作不可恢复，请慎重！是否继续?'), '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'error'
      }).then(() => {
    ...
    confirmStandard() {
      ...
      for(let i = 0; i < this.skuList.length; i++) {
        // 需要删除的sku的sn记录，成功编辑时进行删除操作
        if(this.skuList[i].sku_sn && !this.skuWaitForDelete.includes(this.skuList[i].sku_sn)) {
          this.skuWaitForDelete.push(this.skuList[i].sku_sn);
        }
      }
      ...
      let skuList = getCombination(skuArr); //sku的规格列表
            // 组合sku
      this.skuList = [];
    ...
    deleteBanner(index) {
      if(this.formData.images[index].id) {
        this.$confirm('已存在的图片删除后无法恢复哦，你是否决定执行该删除操作', '提示', {
          ...
          this.$get('/admin/goods/deleteImages', params, res => {
    ...
    deleteSkuItem(scope) {
      if(scope.row.sku_sn) {
        // 旧有sku项，先询问
        let sku_sn = scope.row.sku_sn;
        this.$confirm(`您将删除 ${scope.row.title} , 该删除操作无法被恢复, 若该商品参与了活动也将在活动列表里一并删除, 商品如需参与活动请自行到活动列表里添加。 是否继续?`, '提示', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          type: 'warning'
        }).then(() => {
          this.$get('/admin/goods/deleteSku', {
            goods_sn: this.$route.query.goods_sn,
            sku_sn: sku_sn
          }, res => {
            this.$message.success(`删除成功`);
            this.skuList.splice(scope.$index, 1);
            if(this.skuWaitForDelete.includes(sku_sn)) {
              this.skuWaitForDelete.splice(this.skuWaitForDelete.indexOf(sku_sn), 1);
            }
          });
        }).catch(() => {});
      } else {
    ...
    beforeEdit() {
      if(this.skuWaitForDelete.length > 0) {
        this.$confirm(`你重置了sku列表, 旧有sku将被删除, 若旧有sku参与了活动也将在活动列表里一并删除, 且活动商品将会下架, 如需继续参与活动请重新到活动列表里添加。 是否继续?`, '提示', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          type: 'warning'
        }).then(() => {
    ...
    editItem() {
      ...
      for(let i = 0; i < paramValues.length; i++) {
        if(paramValues[i] == undefined || paramValues[i] == null || paramValues[i] === '') {
          return this.$message.error('请填写/勾选完所有信息，并上传封面哦');
        }
      }
      ...
      for(let i = 0; i < params.sku.length; i++) {
        ...
        if(!currentSku.sku_sn) {
          if(!(/^(0|[1-9][0-9]*)$/.test(currentSku.stock))) {
            return this.$message.error(`第${i + 1}个sku是新增的，它的库存要为0，1，2，3，4这种正整数哦，请检查下哈`)
          }
        }
        ...
        if(currentSku.sku_sn) delete params.sku[i].stock;  //删除库存项，现在不编辑库存了
      }
      ...
      this.$post('/admin/goods/update', params, res => {
        if(this.skuWaitForDelete.length > 0) {
          this.deleteWaitDeleteSku();
        } else {
          this.$message.success('编辑商品成功，正在返回列表');
          setTimeout(() => {
    ...
    deleteWaitDeleteSku() {
      let params = {
        goods_sn: this.$route.query.goods_sn,
        sku_sn: JSON.parse(JSON.stringify(this.skuWaitForDelete))
      }
      this.$get('/admin/goods/deleteSku', params, res => {
        this.$message.success('编辑商品成功，正在返回列表');
        setTimeout(() => {
          localStorage.setItem('shouldRefresh', 'on');
          this.$router.replace('commedities');
        ...
        this.$message.warning('编辑商品成功，但未能正确移除旧有产品，请检查');
      });
    }
