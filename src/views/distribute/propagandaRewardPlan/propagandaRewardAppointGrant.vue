    ...
    <div class="subpage-content">
      <div class="default-container">
        <div class="table-ctrls">
          <div class="table-title">指定用户发放广宣奖</div>
        </div>
      ...
      <div v-for="(item, index) in rules" class="default-container" style="margin-top: 15px" :key="index">
        <div class="table-ctrls">
          <div class="table-title">
            <span>{{item.name}} 用户池</span>
            <span class="per-prize" v-if="item.users && item.users.length > 0">
              预计每人 <span class="success-color">{{item.per_prize}}</span> 元
            </span>
          </div>
...
<script>
  ...
  import SelectUsers from '@/partialviews/distribute/propagandaRewardPlan/SelectUsers';
  export default {
    name: 'propagandaRewardAppointGrant',
    components: {
      SelectUsers
    },
    data() {
    ...
    mounted() {
      ...
      if(!this.plan_sn) {
        this.$message.error('广宣奖计划不存在或已失效，正在返回');
        setTimeout(() => {
          window.location.replace('/#/propagandaRewardPlans');
        }, 2000)
        return;
      }
      if(!this.$auths.includes('propagandaRewardRule/list')) {
        this.$message.warning('您缺少查看广宣奖规则的权限，无法正常指定用户群体发放，请联系管理员为您添加');
        return this.$router.replace('index');
      }
      if(!this.$auths.includes('propagandaRewardPlan/info')) {
        this.$message.warning('您缺少查看广宣奖计划详情的权限，功能异常，请联系管理员为您添加');
        return this.$router.replace('propagandaRewardPlans');
      }
      if(!this.$auths.includes('user/list')) {
        this.$message.warning('您缺少查看用户列表的权限，无法正常选择用户，请联系管理员为您添加');
        return this.$router.replace('propagandaRewardPlans');
      }
      this.getRules();
      this.getData();
    ...
    methods: {
      getRules() {
        ...
        this.$get('/admin/propagandaRewardRule/list', params, res => {
          let rules = res.data;
          for(let i = 0; i < rules.length; i++) {
            rules[i].users = [];

            rules[i].per_prize = '0.00';  //重置每个人分0元
            if(this.planData.total_reward_price) {
              // 计划信息先被请求到了
              let total_prize = parseFloat(this.planData.total_reward_price) * parseFloat(rules[i].scale);
              rules[i].total_prize = total_prize.toFixed(2);
            }
          }
          this.rules = rules;
        });
        ...
        this.$get('/admin/propagandaRewardPlan/info', params, res => {
          ...
          if(this.rules && this.rules.length > 0) {
            ...
            for(let i = 0; i < this.rules.length; i++) {
              let total_prize = parseFloat(this.planData.total_reward_price) * parseFloat(this.rules[i].scale);
              this.rules[i].total_prize = total_prize.toFixed(2);
            }
          }
      ...
      excelSuccess(res, file) {
        ...
        if(res && res.error_code == 0) {
          ...
          this.validErrorUsers(validData, () => {
            ...
            this.calcPerPrize(this.ctrlIndex);
      ...
      calcPerPrize(index) {
        ...
        if(!rule.users || rule.users.length == 0) {
          this.rules[i].per_prize = '0.00';
        } else {
          let total_prize = this.rules[index].total_prize;
          let num = this.rules[index].users.length;
          let per_prize = parseFloat(total_prize) / num;
          this.rules[index].per_prize = per_prize.toFixed(2);
        }
      },
      ...
      confirmGrant() {
        ...
        this.$confirm(`您将对用户池里的总共${userLen}个用户进行派发广宣奖操作, 是否继续?`, '提示', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          type: 'warning'
        }).then(() => {
          
          this.$post('/admin/propagandaReward/grant', params, res => {
            this.$message.success('指定用户发放成功');
            localStorage.setItem('shouldRefresh', 'on');
            this.$router.replace('propagandaRewardPlans');
          });
        }).catch(() => {});
      },
