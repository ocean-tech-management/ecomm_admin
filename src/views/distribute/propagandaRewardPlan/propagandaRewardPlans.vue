  ...
  <div class="subpage-content">
    <div class="default-container">
      ...
      <div class="table-ctrls">
        <div class="table-title">广宣奖计划</div>
        <div class="table-ctrl">
          <template v-if="$auths.includes('propagandaRewardPlan/create')">
      ...
      <el-table style="width: 100%" :data="dataList">
        ...
        <el-table-column label="操作">
         <template slot-scope="scope">
          ...
          <template v-if="$auths.includes('propagandaRewardPlan/delete')">
...
<script>
import Add from '@/partialviews/distribute/propagandaRewardPlan/Add';
import Edit from '@/partialviews/distribute/propagandaRewardPlan/Edit';
import GrantPreview from '@/partialviews/distribute/propagandaRewardPlan/GrantPreview';

export default {
  name: 'propagandaRewardRules',
  components: {
    Add,
    Edit,
    GrantPreview
  ...
  methods: {
    ...
    getList() {
      ...
      this.$get('/admin/propagandaRewardPlan/list', queryVal, res => {
    ...
    addItem() {
      ...
      this.$post('/admin/propagandaRewardPlan/create', params, res => {
    ...
    EditItem() {
      ...
      this.$post('/admin/propagandaRewardPlan/update', params, res => {
    ...
    tryGrant(row) {
      if(!this.$auths.includes('propagandaReward/userList')) {
        return this.$message.warning('无查看 查看广宣奖计划可发放总人数 权限，请联系管理员添加相关权限');
      }
      ...
      this.$get('/admin/propagandaReward/userList', params, res => {
    ...
    confirmGrant() {
      this.$confirm(`您将准备对${this.grantData.plan_name}执行发放操作, 是否继续?`, '提示', {
        ...
        this.$post('/admin/propagandaReward/grant', {
    ...
    deleteItem(row) {
      this.$confirm(`您将删除${row.plan_name}, 是否继续?`, '提示', {
        ...
        this.$post('/admin/propagandaRewardPlan/delete', {
          plan_sn: row.plan_sn
        }, res => {
    ...
    toAppointGrant(row) {
      this.$router.push({path: 'propagandaRewardAppointGrant', query: {plan_sn: row.plan_sn}})
    },
    goToGrantInfo(row) {
      this.$router.push({path: 'propagandaRewardGrantInfo', query: {plan_sn: row.plan_sn}})
    }
