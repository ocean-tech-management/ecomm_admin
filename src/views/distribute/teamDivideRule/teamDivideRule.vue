  ...
  <div class="subpage-content">
    <div class="default-container">
      ...
      <div class="table-ctrls">
        <div class="table-title">团队分润规则</div>
      </div>
      <el-table style="width: 100%" :data="dataList">
        ...
        <el-table-column label="操作">
         <template slot-scope="scope">
          <template v-if="$auths.includes('teamDivide/update')">
...
<script>
import Edit from '@/partialviews/distribute/teamDivideRule/EditRule';
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      this.$get('/admin/teamDivide/list', queryVal, res => {
    ...
    showEditLeaderRule(row) {
      ...
      this.$get('/admin/teamDivide/info', params, res => {
        let data = res.data;
        ...
        this.formData = {
          ...
          village_performance: data.village_performance,

          demotion_sales_price: data.demotion_sales_price,
          demotion_type: data.demotion_type,

          get_reward_condition_price: data.get_reward_condition_price,  //奖励门槛金额

          floors_withdraw_amount: data.floors_withdraw_amount,
          withdraw_condition_amount: data.withdraw_condition_amount,
    ...
    editLeaderRule() {
      ...
      if(
          !/^(\d|[1-9]\d+)(\.\d+)?$/.test(params.village_performance)||
          parseFloat(params.village_performance) < 0
        ) {
        return this.$message.error('小区业绩要为大于等于0的正数哦');
      }
      if(
      ...
      if(
          !/^(\d|[1-9]\d+)(\.\d+)?$/.test(params.get_reward_condition_price)||
          parseFloat(params.get_reward_condition_price) < 0
        ) {
        return this.$message.error('奖励门槛金额要为大于等于0的正数哦');
      }

      
      if(
          !/^(\d|[1-9]\d+)(\.\d+)?$/.test(params.floors_withdraw_amount)||
          parseFloat(params.floors_withdraw_amount) <= 0
        ) {
        return this.$message.error('保底提现额度要为大于0的正数哦');
      }
      if(
          !/^(\d|[1-9]\d+)(\.\d+)?$/.test(params.withdraw_condition_amount)||
          parseFloat(params.withdraw_condition_amount) <= 0
        ) {
        return this.$message.error('提现达标额度要为大于0的正数哦');
      }
      if(
          !/^(\d|[1-9]\d+)(\.\d+)?$/.test(params.withdraw_amount)||
          parseFloat(params.withdraw_amount) <= 0
        ) {
        return this.$message.error('达标提现金额要为大于0的正数哦');
      }
      ...
      params.get_reward_condition_price = parseFloat(params.get_reward_condition_price).toFixed(2);

      this.$post('/admin/teamDivide/update', params, res => {
