      ...
      <div class="default-container">
        ...
        <div v-if="showExtendFilter" class="page-ctrl">
          <el-form :inline="true" :model="queryForm" @submit.native.prevent ref="queryForm">
            ...
            <div>
              <el-form-item label="区轮期">
                <el-select v-model="queryForm.searCrowdKeyArea" filterable clearable placeholder="请选择福利区"
                  @change="remoteSearCrowdKeyMethod(2)">
                  <el-option v-for="item in searCrowdKeyAreaOptions" :key="item.activity_code" :label="item.title"
                    :value="item.activity_code">
                  </el-option>
                </el-select>
                <el-select v-model="queryForm.searCrowdKeyRound" filterable clearable placeholder="请选择福利轮数"
                  @change="remoteSearCrowdKeyMethod(3)">
                  <el-option v-for="item in searCrowdKeyRoundOptions" :key="item.number" :label="item.title"
                    :value="item.number">
                  </el-option>
                </el-select>
                <el-select v-model="queryForm.searCrowdKeyPeriod" filterable clearable placeholder="请选择福利期数">
                  <el-option v-for="item in searCrowdKeyPeriodOptions" :key="item.number" :label="item.title"
                    :value="item.number">
                  </el-option>
                </el-select>
              </el-form-item>
              <el-form-item label="是否熔断" prop="searCrowdFuse">
                <el-select v-model="queryForm.searCrowdFuse" filterable placeholder="请选择">
                  <el-option label="不限" value=""></el-option>
                  <el-option label="是" :value="1"></el-option>
                  <el-option label="否" :value="2"></el-option>
                </el-select>
              </el-form-item>
              <el-form-item v-if="queryForm.searCrowdFuse == 1" label="熔断方案" prop="searCrowdFuseType">
                <el-select v-model="queryForm.searCrowdFuseType" filterable placeholder="请选择">
                  <el-option label="不限" value=""></el-option>
                  <el-option v-for="(value, key) in searCrowdFuseTypeTexts" :label="value" :value="key"
                    :key="key"></el-option>
                </el-select>
              </el-form-item>
              <el-form-item>
        ...
        <el-table style="width: 100%" :data="dataList">
          <el-table-column label="订单号" width="180" prop="order_sn">
            <template slot-scope="{ row }">
              ...
              <template v-if="row.advance_buy == 1">
          ...
          <el-table-column label="下单日期" width="250" prop="create_time">
            <template slot-scope="{ row }">
              <div>{{ row.create_time }}</div>
              <template
                v-if="row.advance_buy == 1 && row.delay_reward_status && row.delay_reward_status != -1 && row.delay_reward_status != -2">
          ...
          <el-table-column label="是否熔断" width="120" align="center">
            <template slot-scope="{row}">
              <div v-if="row.crowd_fuse_status == 1">
                <div>熔断</div>
                <div v-if="row.crowd_fuse_type == 1">方案一</div>
                <div v-if="row.crowd_fuse_type == 2">方案二</div>
                <div v-if="row.crowd_fuse_type == 88">待选择</div>
                <div v-if="(row.crowd_fuse_type == 1 || row.crowd_fuse_type == 2) && row.crowd_fuse_time">{{
                  row.crowd_fuse_time }}</div>
              </div>
              <div v-else>否</div>
            </template>
          </el-table-column>
...
export default {
  ...
  data() {
    return {
      ...
      queryForm: {
        ...
        searCrowdKeyArea: '', // 区
        searCrowdKeyRound: '', // 轮
        searCrowdKeyPeriod: '', // 期

        searCrowdFuse: '', // 是否熔断
        searCrowdFuseType: '', // 熔断方案
      },
      filter: {
        searOrderSn: '',
        order_sign: [],
      ...
      searCrowdKeyLoading: false,
      searCrowdKeyAreaOptions: [],
      searCrowdKeyRoundOptions: [],
      searCrowdKeyPeriodOptions: [],
  ...
  methods: {
    ...
    resetQuery(queryForm) {
      ...
      this.queryForm.searCrowdKeyArea = ''
      this.queryForm.searCrowdKeyRound = ''
      this.queryForm.searCrowdKeyPeriod = ''
     ...
     * @param {*} searType
     */
    remoteSearCrowdKeyMethod(searType = 1) {
      const params = {
        searType,
      }
      if (searType == 2 && !this.queryForm.searCrowdKeyArea) {
        this.queryForm.searCrowdKeyRound = '';
        this.queryForm.searCrowdKeyPeriod = '';
        this.searCrowdKeyRoundOptions = [];
        this.searCrowdKeyPeriodOptions = [];
        return
      }
      if (searType == 3 && !this.queryForm.searCrowdKeyRound) {
        this.queryForm.searCrowdKeyPeriod = '';
        this.searCrowdKeyPeriodOptions = [];
        return
      }
      if (searType == 2) {
        params.activity_code = this.queryForm.searCrowdKeyArea;
        this.queryForm.searCrowdKeyRound = '';
        this.queryForm.searCrowdKeyPeriod = '';
        this.searCrowdKeyRoundOptions = [];
        this.searCrowdKeyPeriodOptions = [];
      ...
      if (searType == 3) {
        params.activity_code = this.queryForm.searCrowdKeyArea;
        params.round_number = this.queryForm.searCrowdKeyRound;
        this.queryForm.searCrowdKeyPeriod = '';
        this.searCrowdKeyPeriodOptions = [];
      }
      this.$get('/admin/crowdFunding/sear', params, res => {
        const data = res.data || [];
        if (searType == 1) this.searCrowdKeyAreaOptions = data;
        if (searType == 2) this.searCrowdKeyRoundOptions = data.map(e => {
          return {
            number: e.number,
            title: `第${e.number}轮`,
        ...
        if (searType == 3) this.searCrowdKeyPeriodOptions = data.map(e => {
          return {
            number: e.number,
            title: `第${e.number}期`,
            activity_code: e.activity_code
    ...
    queryInfo() {
      ...
      this.filter = {
        ...
        searCrowdKeyArea: this.queryForm.searCrowdKeyArea,
        searCrowdKeyRound: this.queryForm.searCrowdKeyRound,
        searCrowdKeyPeriod: this.queryForm.searCrowdKeyPeriod,

        searCrowdFuse: this.queryForm.searCrowdFuse,
        searCrowdFuseType: this.queryForm.searCrowdFuseType,
      }
      this.getList();
    },
    ...
    getList() {
      ...
      if(this.showExtendFilter) {
        ...
        queryVal.searCrowdFuse = this.filter.searCrowdFuse;
        queryVal.searCrowdFuseType = this.filter.searCrowdFuseType;

        if (this.filter.searCrowdKeyArea || this.filter.searCrowdKeyRound || this.filter.searCrowdKeyPeriod) {
          if (!this.filter.searCrowdKeyArea) return this.$message.error('请优先选择福利区');
          if (this.filter.searCrowdKeyArea && !this.filter.searCrowdKeyRound && this.filter.searCrowdKeyPeriod) return this.$message.error('请优先选择福利轮数');
          let searCrowdKey = this.filter.searCrowdKeyArea
          searCrowdKey += this.filter.searCrowdKeyRound ? `-${this.filter.searCrowdKeyRound}` : ''
          searCrowdKey += this.filter.searCrowdKeyPeriod ? `-${this.filter.searCrowdKeyPeriod}` : ''
          queryVal.searCrowdKey = searCrowdKey
        }
      ...
      this.$post('/admin/crowdFunding/orderList', queryVal, res => {
    ...
    sgetList() {
      ...
      this.dataLoading = true;
      this.$spost('/admin/crowdFunding/orderList', queryVal, res => {
        let dataList = res.data.list || [];
    ...
    extendFilter() {
      ...
      if (this.searCrowdKeyAreaOptions.length === 0) this.remoteSearCrowdKeyMethod(1)
    },
    showSelectGoods() {
    ...
    exportExcel() {
      ...
      if(this.showExtendFilter) {
        ...
        params.searCrowdFuse = this.filter.searCrowdFuse;
        params.searCrowdFuseType = this.filter.searCrowdFuseType;
        ...
        if (this.filter.searCrowdKeyArea || this.filter.searCrowdKeyRound || this.filter.searCrowdKeyPeriod) {
          ...
          searCrowdKey += this.filter.searCrowdKeyRound ? `-${this.filter.searCrowdKeyRound}` : ''
          searCrowdKey += this.filter.searCrowdKeyPeriod ? `-${this.filter.searCrowdKeyPeriod}` : ''
          params.searCrowdKey = searCrowdKey
        }
      ...
      this.$post('/admin/excel/order', params, res => {
        ...
        for(let i = 0; i < data.length; i++) {
          ...
          excelData.push([
            ...
            teamLeader.name,
            teamLeader.phone,
            item.searCrowdFuse == 1 ? '是' : '否',
            item.searCrowdFuse == 1 ? this.searCrowdFuseTypeTexts[item.searCrowdFuseType] : '',
            item.crowd_fuse_time || ''
          ]);
        }
