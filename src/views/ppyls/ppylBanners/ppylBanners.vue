  ...
  <div class="subpage-content">
    <div class="default-container">
      ...
      <div class="table-ctrls">
        <div class="table-title">拼拼广告列表</div>
        <div class="table-ctrl">
          <template v-if="$auths.includes('ppylBanner/create')">
      ...
      <el-table style="width: 100%" :data="dataList">
        ...
        <el-table-column label="拼拼广告名称" prop="title" width="250"></el-table-column>
        ...
        <el-table-column label="操作" >
          <template slot-scope="scope">
            <template v-if="$auths.includes('ppylBanner/status')">
            ...
            <template v-if="$auths.includes('ppylBanner/update')">
              <el-button type="text" size="small" @click="openEdit(scope.row)">修改</el-button>
            </template>
            <template v-if="$auths.includes('ppylBanner/delete')">
    ...
    <el-dialog title="新建拼拼广告" width="800px" :close-on-click-modal="false" :visible.sync="addVisible">
    ...
    <el-dialog title="编辑拼拼广告" :close-on-click-modal="false" width="950px" :visible.sync="editVisible">
...
import Add from '@/partialviews/ppyl/banner/AddBanner';
import Edit from '@/partialviews/ppyl/banner/EditBanner';
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      this.$get('/admin/ppylBanner/list', queryVal, res => {
    ...
    addItem() {
      ...
      if(!/^(0|[1-9][0-9]*)$/.test(params.sort)) {
        return this.$message.error('排序要为1，2，3，4这种正整数');
      }
      ...
      this.$post('/admin/ppylBanner/create', params, res => {
        this.$message.success('添加拼拼广告成功');
        this.addVisible = false;
        this.reloadPage();
      });
    },
    openEdit(row) {
      this.$get('/admin/ppylBanner/info', {
        id: row.id
      }, res => {
        let data = res.data;
        this.formData = {
    ...
    editItem() {
      ...
      for(let i = 0; i < paramValues.length; i++) {
        if(paramValues[i] == undefined || paramValues[i] == null || paramValues[i] == '') {
          return this.$message.error('请填写完所有信息，并上传拼拼广告哦');
        }
      }
      if(!/^(0|[1-9][0-9]*)$/.test(params.sort)) {
      ...
      this.$post('/admin/ppylBanner/update', params, res => {
        this.$message.success('编辑拼拼广告成功');
    ...
    toggleItem(row) {
      this.$confirm(`您将${row.status == 1 ? '停用' : '启用'}${row.title}, 是否继续?`, '提示', {
        ...
        this.$post('/admin/ppylBanner/status', {
          id: row.id,
          status: row.status
        }, res => {
    ...
    deleteItem(row) {
      this.$confirm(`您将删除${row.title}, 是否继续?`, '提示', {
        ...
        this.$get('/admin/ppylBanner/delete', {
          id: row.id
        }, res => {
