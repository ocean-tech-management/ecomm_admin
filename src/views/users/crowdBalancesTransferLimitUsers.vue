  ...
  <div class="subpage-content">
    <div class="default-container">
      ...
      <div class="table-ctrls">
        <div class="table-title">集品券互转白名单列表</div>
        <div class="table-ctrl">
          <template v-if="$auths.includes('user/banCrowdTransfer')">
            <el-button type="danger" @click="cancelAllWhiteList">清空白名单</el-button>
          </template>
          <template v-if="$auths.includes('user/banCrowdTransfer')">
          ...
          <template slot-scope="scope">
            <template v-if="$auths.includes('user/banCrowdTransfer')">
              <el-button class="error-color" type="text" size="small"
                @click="cancelBanCrowdTransfer(scope.row)">取消白名单</el-button>
            </template>
          </template>
        </el-table-column>
      </el-table>
    ...
    <el-dialog title="新增白名单人员" width="800px" :close-on-click-modal="false" :visible.sync="assignVisible">
...
<script>
import Assign from '@/partialviews/user/crowdBalancesTransferLimitUser/Assign';

export default {
  name: 'crowdBalancesTransferLimitUsers',
  components: {
    Assign
  },
  data() {
  ...
  methods: {
    ...
    getList() {
      ...
      this.$get('/admin/user/banTransferUserList', queryVal, res => {
    ...
    cancelAllWhiteList() {
      ...
      this.$confirm('是否确定清空白名单，该操作将使所有用户无法使用集品券互转功能，请确定', '提示', {
        ...
        this.$post('/admin/user/banCrowdTransfer', params, res => {
          this.$message.success('清空白名单成功');
          this.reloadPage();
        });
      }).catch(() => {});
    },

    cancelBanCrowdTransfer(row) {
      this.$confirm(`是否确定移除 ${row.name} 的互转白名单的身份？`, '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
