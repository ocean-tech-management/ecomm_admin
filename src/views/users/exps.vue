  ...
  <div class="subpage-content">
    <div class="default-container">
      ...
      <div class="table-ctrls">
        <div class="table-title">体验中心列表</div>
        <div class="table-ctrl">
          <el-button type="primary" @click="exportExcel">导出数据</el-button>
          <template v-if="$auths.includes('exp/assign')">
            <el-button type="primary" @click="showVipAssignPopup(1)">指定体验中心</el-button>
          </template>
        </div>
      </div>
      <el-table style="width: 100%" :data="dataList">
        ...
        <el-table-column label="身份" width="140">
          <template slot-scope="scope">
            {{leaderTexts[scope.row.exp_level] || '——'}}
          </template>
        </el-table-column>
        <el-table-column label="指定时间" prop="exp_upgrade_time" width="150"></el-table-column>
    ...
    <el-dialog title="指定体验中心" width="800px" :close-on-click-modal="false" :visible.sync="vipAssignVisible">
...
import Assign from '@/partialviews/user/exp/Assign';
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      this.$get('/admin/exp/list', queryVal, res => {
    ...
    getVdc() {
      this.$get('/admin/expVdc/list', null, res => {
    ...
    vipAssign() {
      ...
      let params = {
        ...
        exp_level: this.formData.exp_level,
        is_exp: this.formData.is_exp
      }
      this.$confirm(`是否确定${params.is_exp == 1 ? '指定用户体验中心身份' : '取消用户体验中心身份'}操作`, '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
        
        this.$post('/admin/exp/assign', params, res => {
          this.$message.success(`${params.is_exp == 1 ? '指定' : '取消'}体验中心成功`);
    ...
    cancelLevel(row) {
      ...
      this.$confirm('是否取消该用户的体验中心身份', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
        ...
        this.$post('/admin/exp/assign', params, res => {
          this.$message.success('取消体验中心身份成功');
    ...
    exportExcel() {
      ...
      this.$get('/admin/exp/list', params, res => {
        ...
        for(let i = 0; i < data.length; i++) {
          ...
          excelData.push([
            item.name, 
            item.phone, 
            this.leaderTexts[item.exp_level] || '——', 
            item.exp_upgrade_time
          ]);
        }
        ...
        XLSX.utils.book_append_sheet(wb, ws, "体验中心列表");

        let time = moment().format('YYYYMMDDHHmm');
        /* generate file and send to client */
        XLSX.writeFile(wb, `体验中心列表${time}.xlsx`);
      });
    },
