    ...
    <div class="default-container">
      <div class="page-ctrl">
        <el-form :inline="true" :model="queryForm" @submit.native.prevent ref="queryForm">
          <el-form-item label="用户关键词" prop="user_keyword">
            <el-input v-model="queryForm.user_keyword" placeholder="请输入用户关键词" @keyup.enter.native="resetQuery"></el-input>
          </el-form-item>
          <el-form-item label="订单关键词" prop="keyword">
      ...
      <el-table style="width: 100%" :data="dataList">
        ...
        <el-table-column label="数量" width="120" prop="number"></el-table-column>
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      this.$get('/admin/advance/rechargeList', queryVal, res => {
    ...
    onUploadSuccess({ selectResult }) {
      this.$confirm(`导入成功，您将对导入的${selectResult.length}条记录进行充值操作, 是否继续?`, '提示', {
        ...
        this.$post('/admin/advance/recharge', {
    ...
    exportExcel() {
      ...
      this.$get('/admin/advance/rechargeList', params, res => {
        ...
        XLSX.utils.book_append_sheet(wb, ws, "集品卡充值列表");

        let time = moment().format('YYYYMMDDHHmm');
        /* generate file and send to client */
        XLSX.writeFile(wb, `集品卡充值列表${time}.xlsx`);
      });
    },
