...
<script>
  export default {
    ...
    methods: {
      resetSelectUsers(selectUsers = [], existCouldNotSelectUsers = []) {
        this.selectUsers = JSON.parse(JSON.stringify(selectUsers));
        this.existCouldNotSelectUsers =  JSON.parse(JSON.stringify(existCouldNotSelectUsers));    
      },
      queryInfo() {
        this.filter = {
      ...
      confirmUser() {
        ...
        let hasSelect = false;
        if(this.existCouldNotSelectUsers.includes(this.users[this.userIndex].uid)) {
          return this.$message.warning('用户存在于其他池子');
        }
