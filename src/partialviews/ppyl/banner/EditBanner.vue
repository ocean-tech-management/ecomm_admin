  ...
  <div>
    <el-form :model="formData" label-width="100px">
      <el-form-item label="广告位名称">
        <el-input style="width: 450px" auto-complete="off" placeholder="填一下新广告位的名称呗" v-model="formData.title"></el-input>
      </el-form-item>
      <el-form-item label="关联拼拼活动">
        <el-select placeholder="请选择" name="activity_code" v-model="formData.activity_code">
          <el-option v-for="item in ppyls" :label="item.activity_title" :value="item.activity_code" :key="item.activity_code"></el-option>
        </el-select>
      </el-form-item>
      <!-- 不让跳转 -->
      <el-form-item v-if="false" label="跳转类型">
        <el-select placeholder="请选择跳转类型" name="type" v-model="formData.type" @change="confirmType(false)">
          <el-option label="仅做展示，不跳转" :value="-1"></el-option>
        ...
        <div
          ...
          >您拥有的权限不同，可以创建的广告图跳转类型可能会有不一样的可选项哦</div>
      </el-form-item>
      ...
      <el-form-item label="展示图">
        ...
        <div
              style="color: #999; font-size:14px; line-height: 1.4; margin-top: 10px"
            >请上传尺寸为750px*880px的图片，不超过2M</div>
      </el-form-item>
      <el-form-item v-if="formData.type == 1" label="内容">
        <el-input auto-complete="off" placeholder="填一下新广告位的内容呗" v-model="formData.content"></el-input>
      </el-form-item>
      <el-form-item label="排序">
        <el-input style="width: 450px" auto-complete="off" type="number" placeholder="填一下新广告位的排序呗，必须是正整数哦" v-model="formData.sort"></el-input>
      </el-form-item>
            ...
            <el-form-item label-width="0px">
              <div class="discount-commodity-select">
                <div class="commodity-select-wrapper commodity-spus">
                  <div v-for="(item, index) in spu" :class="{'commodity-select-item': true, 'active': spuIndex == index}" :key="item.goods_sn" @click="selectSpu(index)" @dblclick="confirmGoods">{{item.title}}</div>
                </div>
    ...
    <template v-if="formData.type == 13">
      <el-form label-width="100px" style="margin-top: 60px;">
        <el-form-item label="选择直播间" prop="content">
          <el-select placeholder="输入关键词筛选" name="content" filterable remote v-model="formData.content" :loading="liveLoading" :remote-method="updateSelectKeyword">
    ...
    <!-- 拼拼活动列表 -->
    <template v-if="formData.type == 19">
      <el-form label-width="100px" style="margin-top: 60px;">
    ...
    <!-- 限时专场列表 -->
    <template v-if="formData.type == 22">
      <el-form label-width="100px" style="margin-top: 60px;">
        <el-form-item label="选择限时专场" prop="content">
          <el-select placeholder="输入关键词筛选" name="content" filterable remote v-model="formData.content" :loading="specialAreaLoading" :remote-method="updateSelectKeyword">
...
export default {
  ...
  methods: {
    ...
    confirmType(notRefresh = false) {
      ...
      } else if(this.formData.type == 19) {
        // 指定拼拼活动
        if(!this.ppyls || this.ppyls.length == 0) this.getPpyls();
      } else if(this.formData.type == 22) {
        // 指定限时专场
    ...
    getLives(keyword = '') {
      ...
      this.$get('/admin/live/roomList', params, res => {
        ...
        for(let i = 0; i < dataList.length; i++) {
          dataList[i].roomid = dataList[i].roomid;
        }
        this.lives = dataList;
        this.$forceUpdate();
