<template>
  <div>
    <el-form :model="formData">
      ...
      <el-form-item label="足迹记录" label-width="100px">
        <div class="steps-box">
          <div v-if="!stepsData || stepsData.length === 0">该用户暂无足迹记录</div>
          <el-timeline>
            <el-timeline-item
              v-for="(step, index) in stepsData"
              :key="index"
              :timestamp="step.enter_time">
              <span class="tips-color">{{step.content}}</span>
              <span v-if="step.goods_title" class="success-color"> {{step.goods_title}} </span>
              <span v-if="step.linker_name"><span class="tips-color">； 来自</span>{{step.linker_name}}</span>
              <span v-if="step.linker_phone">({{step.linker_phone}})</span>
              <span v-if="step.type == 1">——{{sceneTexts[step.entrance_type]}}</span>
            </el-timeline-item>
          </el-timeline>
        </div>
      </el-form-item>
