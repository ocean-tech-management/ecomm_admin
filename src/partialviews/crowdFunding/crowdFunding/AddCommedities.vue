  ...
  <div>
    <div class="category-ctrl">
      <el-form :inline="true" :model="queryForm" @submit.native.prevent ref="categorySelect" class="demo-form-inline">
        ...
        <el-form-item label="品牌" prop="brand_code">
          <el-select placeholder="请输入品牌名筛选" name="brand_code" filterable remote :loading="brandLoading"
            v-model="queryForm.brand_code" :remote-method="updateBrandCode">
            <el-option v-for="item in brands" :label="item.brand_name" :value="item.brand_code"
              :key="item.brand_code"></el-option>
          </el-select>
        </el-form-item>
        <el-form-item>
...
export default {
  ...
  methods: {
      ...
      getBrands(keyword = '') {
        if(!this.$auths.includes('brand/list')) {
          return;
        }
        let params = {
          keyword: keyword
        ...
        this.$sget('/admin/brand/list', params, res => {
      ...
      queryInfo(){
        ...
        let params = {
          ...
          brand_code: this.queryForm.brand_code,
          searType: 2
        };
      ...
      updateBrandCode(keyword) {
        this.getBrands(keyword);
      },
