  ...
  <div>
    <el-form :model="formData" label-width="160px">
      ...
      <el-form-item label="重开时间间隔">
        <el-input v-model="formData.reopen_time_interval" type="number" disabled style="width: 250px;">
          <span slot="suffix" style="padding: 0 5px;">天</span>
        </el-input>
      </el-form-item>
      <el-form-item label="涨幅间隔期数">
        <el-input v-model="formData.rise_period_number" type="number" disabled style="width: 250px;">
          <span slot="suffix" style="padding: 0 5px;">期</span>
        </el-input>
      </el-form-item>
      <el-form-item label="每期涨幅">
        <el-input v-model="formData.rise_scale" disabled style="width: 250px;">
          <span slot="suffix" style="padding: 0 5px;">%</span>
        </el-input>
      </el-form-item>
      <el-form-item label="赠送礼物类型">
        <el-select v-model="formData.gift_type" placeholder="赠送礼物类型" @change="onGiftChange">
      ...
      <div v-show="showGiftTypeTips" class="tips">
        <strong>若变更赠送礼物类型, 该区对应的所有期的商品 赠送的新礼物类型 数量都将重置为0, 确认变更赠送类型后请立即修改对应期商品的赠送数量!</strong>
      </div>
      <el-form-item label="用户参与预售次数限制">
        <el-input v-model="formData.user_advance_limit" auto-complete="off" placeholder="大于0的整数" maxlength="9"
          style="width: 250px;">
          <span slot="suffix" style="padding: 0 5px;">次</span>
        </el-input>
      </el-form-item>
      <el-form-item label="预售自动新增期数限制">
        <el-input v-model="formData.auto_create_advance_limit" auto-complete="off" placeholder="大于0的整数" maxlength="9"
          style="width: 250px;">
          <span slot="suffix" style="padding: 0 5px;">期</span>
        </el-input>
      </el-form-item>
...
export default {
  ...
  data() {
    return {
      ...
      thumbData: null,
      showGiftTypeTips: false, // 是否显示更换怎送礼物类型提示
    };
  },
  props: {
    formData: Object,
  ...
  methods: {
    reset() {
      this.showGiftTypeTips = false;
      this.$refs.background_image && this.$refs.background_image.clearFiles();
    },
    imageWatcher(file) {
    ...
    // 切换赠送礼物类型
    onGiftChange(val) {
      if (this.defaultGiftType !== '') {
        // 根据默认赠送类型判断
        this.showGiftTypeTips = this.defaultGiftType !== val;
      } else {
        this.showGiftTypeTips = true;
      }
    },
  }
};
