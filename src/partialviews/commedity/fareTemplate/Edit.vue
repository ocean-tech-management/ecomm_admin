<template>
  <div>
        ...
        <div class="template-content">
          ...
          <div class="template-content-extends">
            <template v-if="formData.extends && formData.extends.length > 0">
              <el-table style="width: 100%" :data="formData.extends">

                <el-table-column label="运送到" prop="names">
                ...
                <el-table-column label="操作" width="120">
                  <template slot-scope="scope">
                    ...
                    <el-button v-if="!scope.row.id" class="warning-color" type="text" size="small" @click="deleteItem(scope.$index)">删除</el-button>
                    <template v-if="$auths.includes('ship/templateDetailDelete')">
                      <el-button v-if="scope.row.id" class="error-color" type="text" size="small" @click="deleteExistItem(scope)">删除</el-button>
                    </template>
                  </template>
                </el-table-column>
    ...
    <el-dialog :append-to-body="true" title="选择指定城市" width="750px" :visible.sync="cityVisible">
      <CityList ref="cityPopup" :selectCity="selectCity" :type="ctrlType" :statusCityData="statusCityData"></CityList>
...
export default {
  ...
  mounted() {
    ...
    this.initAllSelectNames();
    this.initAllSelectLimitInfo();
    // this.getCities(() => {
    // });
  ...
  methods: {
    getCities(handler) {
      ...
      this.$post('/admin/ship/city', params, (res) => {
        ...
        for(let i = 0; i < cityData.length; i++) {
          cityNames[cityData[i].code] = cityData[i].name;
          for(let j = 0; j < cityData[i].city.length; j++) {
            cityNames[cityData[i].city[j].code] = cityData[i].city[j].name;
          }
        }
    ...
    initAllSelectNames() {
      ...
      for(let i = 0; i < this.formData.extends.length; i++) {
        extendItems.push(JSON.parse(JSON.stringify(this.formData.extends[i])));
      }
      for(let i = 0; i < extendItems.length; i++) {
        if(extendItems[i].city_code == '') return extendItems[i].names = '';
        let selectResults = {};
        for(let j = 0; j < extendItems[i].city_code.length; j++) {
          let item = extendItems[i].city_code[j];
          let provinceId = item.substring(0, 2);
          if(!selectResults[provinceId]) selectResults[provinceId] = [];
          selectResults[provinceId].push(item);
        }
        extendItems[i].names = this.getSelectCityResultName(selectResults);
      }
      this.formData.extends = [];
      this.formData.extends = extendItems;
      this.$forceUpdate();
    },
    
    initAllSelectLimitInfo() {
      let selectLimitResult = {};
      let ids = [];
      for(let i = 0; i < this.formData.notSaleArea.codes.length; i++) {
        // 2个字符是省
        if(this.formData.notSaleArea.codes[i].length == 2) {
          let provinceId = this.formData.notSaleArea.codes[i];
          //省，全选市和区
          for(let j = 0; j < this.cityData.length; j++) {
            // 省
            if(this.cityData[j].code == provinceId) {
              if(!selectLimitResult[provinceId]) selectLimitResult[provinceId] = {};
              for(let k = 0; k < this.cityData[j].city.length; k++) {
                // 市
                let cityId = this.cityData[j].city[k].code;
                if(!selectLimitResult[provinceId][cityId]) selectLimitResult[provinceId][cityId] = [];
                for(let l = 0; l < this.cityData[j].city[k].area.length; l++) {
                  // 区
                  let areaId = this.cityData[j].city[k].area[l].code;
                  selectLimitResult[provinceId][cityId].push(areaId);
                  ids.push(areaId);
                }
              }
            }
          }
        } else if(this.formData.notSaleArea.codes[i].length == 4) {
          // 4个字符是市,其下所有区全选
          let cityId = this.formData.notSaleArea.codes[i];
          let provinceId = cityId.substring(0, 2);
          if(!selectLimitResult[provinceId]) selectLimitResult[provinceId] = {};
          if(!selectLimitResult[provinceId][cityId]) selectLimitResult[provinceId][cityId] = [];
          for(let j = 0; j < this.cityData.length; j++) {
            // 省
            if(this.cityData[j].code == provinceId) {
              for(let k = 0; k < this.cityData[j].city.length; k++) {
                // 市
                if(this.cityData[j].city[k].code == cityId) {
                  for(let l = 0; l < this.cityData[j].city[k].area.length; l++) {
                    // 区
                    let areaId = this.cityData[j].city[k].area[l].code;
                    selectLimitResult[provinceId][cityId].push(areaId);
        ...
        } else if(this.formData.notSaleArea.codes[i].length == 6) {
          // 6个字符是区,直接插入
          let areaId = this.formData.notSaleArea.codes[i];
          let cityId = areaId.substring(0, 4);
          let provinceId = areaId.substring(0, 2);
          if(!selectLimitResult[provinceId]) selectLimitResult[provinceId] = {};
          if(!selectLimitResult[provinceId][cityId]) selectLimitResult[provinceId][cityId] = [];
          selectLimitResult[provinceId][cityId].push(areaId);
          ids.push(areaId);
      ...
      this.selectLimitResult = selectLimitResult;
      this.formData.notSaleArea.ids = ids;
      this.$forceUpdate();
    },
    ...
    deleteExistItem(scope) {
      this.$confirm(`该城市项为旧有项，删除将无法还原，是否继续?`, '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
        this.$post('/admin/ship/templateDetailDelete', {
          id: scope.row.id
        }, res => {
          this.formData.extends.splice(scope.$index, 1);
          this.formData.extends = this.formData.extends.slice(0);
    ...
    showAddItem() {
      ...
      if(this.$refs.cityPopup) {
        // 已存在弹窗，强制更新弹窗信息
        this.$refs.cityPopup.$forceUpdate();
        this.$refs.cityPopup.resetResult();
      }
    ...
    showEditItem(index) {
      ...
      if(this.$refs.cityPopup) {
        // 已存在弹窗，强制更新弹窗信息
        this.$refs.cityPopup.$forceUpdate();
        this.$refs.cityPopup.resetResult(selectInfos);
        this.statusCityData = statusCityData;
        this.cityVisible = true;
      } else {
        this.statusCityData = statusCityData;
    ...
    editItem() {
      ...
      if(currentItem.id) item.id = currentItem.id;
